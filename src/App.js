...
class App extends Component {
    ...
    themeListRenderer() {
        ...
        return themeList.map((t, k) =>
            <button key={k} data-theme={t} onKeyDown={this.keyboardNavigation}
                    className={`theme-button bg-${t}-500${theme === t ? ' is-active' : ''}`}
                    onClick={this.changeTheme}></button>
        )
    }
        ...
        return Object.entries(iconList).map(([type, icons]) =>
            <div className="blocks" key={type}>
                ...
                <div className="block-list">
                    {Object.entries(icons).map(icon => <button key={icon[0]} tabIndex="0" onClick={this.changeBlock}
                                                               className={`block-item${icon[0] === blockName ? ' is-active' : ''}`}
                                                               block-type={type}
                                                               block-name={icon[0]}>{icon[1]}</button>)}
                </div>
            </div>
        );
    }
    ...
    viewModeRenderer() {
        ...
        return viewList.map((v, k) => <button key={k} className={`device${view === v.name ? ' is-active' : ''}`}
                                              data-view={v.name} onClick={this.changeView}>{v.icon}</button>);
    }
    ...
    render() {
        ...
        return (
            <div className={`app${darkMode ? ' dark-mode' : ''}${sidebar ? ' has-sidebar' : ''} ${theme} ${view}`}>
                            ...
                            head={
                                <>
                                    <link
                                        ...
                                        rel="stylesheet"/>
                                    {darkMode ? <style
                                            dangerouslySetInnerHTML={{__html: `img { filter: invert(1); mix-blend-mode: color-dodge }`}}/> :
                                        <style
                                            dangerouslySetInnerHTML={{__html: `img { filter: sepia(1) hue-rotate(180deg) opacity(.9) grayscale(.7) }`}}/>}
                                </>
                            }
                        >
                ...
                <a href="https://github.com/mertJF/tailblocks" className="github" target="_blank"
                ...
                <div className="keyboard-nav">
                    <div className={`k-up keyboard-button${currentKeyCode === 38 ? ' is-active' : ''}`}
                         data-info="Previous block">
                        <svg stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round"
                             strokeLinejoin="round" viewBox="0 0 24 24">
                            <path d="M12 19V5M5 12l7-7 7 7"/>
                        </svg>
                    </div>
                    <div className="keyboard-nav-row">
                        <div className={`k-left keyboard-button${currentKeyCode === 37 ? ' is-active' : ''}`}
                        ...
                        <div className={`k-down keyboard-button${currentKeyCode === 40 ? ' is-active' : ''}`}
                        ...
                        <div className={`k-right keyboard-button${currentKeyCode === 39 ? ' is-active' : ''}`}
